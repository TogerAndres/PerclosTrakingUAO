...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        blink_history.append(blink_frequency)
...
# Guardar gráfico del comportamiento
if time_history and blink_history:
    base_time = time_history[0]
    time_rel = [(t - base_time) / 60 for t in time_history]  # minutos

    plt.figure(figsize=(10, 5))
    plt.plot(time_rel, blink_history, label='Parpadeos por minuto', color='blue', linewidth=2)
    plt.xlabel("Tiempo (minutos)")
    plt.ylabel("Frecuencia de parpadeos")
    plt.title("Comportamiento del parpadeo a lo largo del tiempo")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("comportamiento.png")
    plt.close()
